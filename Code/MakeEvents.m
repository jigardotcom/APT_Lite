...
function [events] = MakeEvents(ExpLog, TemporalError, T_Delta)
...
for i = 1:(size(SessionIndices,2))
    ...
    Logticks_Session = Logticks(SessionIndices(i).start:SessionIndices(i).end,1);
    LogEventSeq_Session = LogEventSeq(SessionIndices(i).start:SessionIndices(i).end,1);
